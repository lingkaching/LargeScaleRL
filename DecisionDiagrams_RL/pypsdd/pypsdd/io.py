...
def my_sdd_read(filename,manager,save_name):
    ...
    for line in f:
        if line.startswith('c'): continue
            ...
            node_id = int(line[2:])
            my_node = my_SddNode(None, int(node_id), 0, None, None)
            my_SddNodes.append(my_node)
        elif line.startswith('T'):
            node_id = int(line[2:])
            my_node = my_SddNode(None, int(node_id), 1, None, None)
            my_SddNodes.append(my_node)
        elif line.startswith('L'):
            node_id,vtree_id,lit = [ int(x) for x in line[2:].split() ]
            my_node = my_SddNode(int(vtree_id), int(node_id), 2, lit, None)
            my_SddNodes.append(my_node)
        elif line.startswith('D'):
            line = [ int(x) for x in line[2:].split() ]
            node_id,vtree_id,size = line[:3]            
            my_elements = line[3:]
            my_elements = [ my_element for my_element in pairs(my_elements) ]
            my_node = my_SddNode(int(vtree_id), int(node_id), 3, None, my_elements)
            my_SddNodes.append(my_node)
    f.close()
    ...
    for my_node in my_SddNodes:
        vtree_id_list.append(my_node.vtree_id)
        node_id_list.append(my_node.node_id)
        node_type_list.append(my_node.node_type)
        elements_list.append(my_node.elements)
        literal_list.append(my_node.literal)

    save_path = save_name+'node_id_list.json'
    with open(save_path, 'w') as f:
        json.dump(node_id_list,f)
    save_path = save_name+'node_type_list.json'
    with open(save_path, 'w') as f:
        json.dump(node_type_list, f)
    save_path = save_name+'elements_list.json'
    with open(save_path, 'w') as f:
        json.dump(elements_list, f)
    save_path = save_name+'literal_list.json'
    with open(save_path, 'w') as f:
        json.dump(literal_list, f)
    save_path = save_name+'vtree_id_list.json'
    with open(save_path, 'w') as f:
        json.dump(vtree_id_list, f)
...
def psdd_yitao_read(filename,pmanager,save_name):
    ...
    for line in f:
        ...
        if line.startswith('c'): continue
            ...
            nodes = {}
        elif line.startswith('F'): # no FALSE node
            my_node = my_SddNode(None, int(node_id), 0, None, None)
            my_SddNodes.append(my_node)
            pass
        elif line.startswith('T'):
            line = line[2:].split()
            # print [float(x) for x in line[:-1]]
            node_id, vtree_id, var, log_theta1 = [ float(x) for x in line[:-1] ]
            # theta = float(line[-1])
            vtree = var_to_vtree[var]
            node = PSddNode(SddNode.TRUE,None,vtree,pmanager)
            node.theta = [math.exp(log_theta1), 1-math.exp(log_theta1)]
            # node.theta = [ 1.0-math.exp(theta),math.exp(theta) ]
            node.theta_sum = sum(node.theta)

            my_node = my_SddNode(int(vtree_id), int(node_id), 1, None, None)
            my_SddNodes.append(my_node)
        elif line.startswith('L'):
            node_id,vtree_id,lit = [ int(x) for x in line[2:].split() ]
            ...
            my_elements = []
            for i in xrange(size):
                my_p = int(line_iter.next())
                my_s = int(line_iter.next())
                my_log_theta = float(line_iter.next())
                my_elements.append([my_p,my_s])
