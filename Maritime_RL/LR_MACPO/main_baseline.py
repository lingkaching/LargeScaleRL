...
from parameters import constraintBeta, SAMPLE_AVG, TOTAL_RUNTIME, EPISODES, SHOULD_LOG, BATCH_SIZE, SAVE_MODEL, LOAD_MODEL, RENDER, wDelay, wResource, capRatio, DISCOUNT, succProb, MAP_ID, MODES, ARR_HORIZON, TOTAL_VESSEL, HORIZON, EVAL_EPISODES, ENV_VAR, OPENBLAS, OMP, MKL, NUMEXPR
...
dirName = str(MAP_ID)+"_"+str(capRatio)+"_"+str(HORIZON)+"_"+str(TOTAL_VESSEL)+"_"+str(constraintBeta)+"_"+args.agent
FileName = str(MAP_ID)+"_"+str(capRatio)+"_"+str(HORIZON)+"_"+str(TOTAL_VESSEL)+"_"+str(constraintBeta)+"_"+args.agent+"_run"+args.runID
...
def init():
    ...
    os.system("mkdir -p ./log/" + dirName + "/data/"+FileName)
    os.system("mkdir ./log/" + dirName + "/model")
    # ax.createLog()
...
def main():
    ...
    for i_episode in range(1, EPISODES+1):
        ...
        for t in range(HORIZON):
            ...
            t1b = ax.now()
            if cT.nt_z[-1] == TOTAL_VESSEL and done_flag:
                beta = np.expand_dims(buffer_beta[t-1], 0)
            else:
                beta = mpa.getBeta(cT, i_episode)

            if cT.nt_z[-1] == TOTAL_VESSEL:
                done_flag = True

            #future work: to learn cost functions
            learnedCost = np.ones(totalZONES)

            # print "action time", tmp2
            actionTime += ax.getRuntime(t1b, ax.now())
            ...
            Rt, RtHat, Rt_z, RtHat_z, cT_new , vioPen, vioPenHat, delayPen = env.step(t, cT, beta, learnedCost, mpa.myLambda)
            sampling += ax.getRuntime(t2, ax.now())
            # print "sampling ", tmp1
        ...
        rollTime += ax.getRuntime(t3, ax.now())
        ...
        if batchID % BATCH_SIZE == 0:
            ...
            trainTime += ax.getRuntime(t4, ax.now())
            # print "Train", ax.getRuntime(t4, ax.now())
            mpa.clear()
            batchID = 0
            mpa.epoch += 1

            # print(np.max(buffer_nt_z, axis=0))
        ...
        if i_episode % SHOULD_LOG == 0:
            ...
            mpa.logStat(i_episode, epVioCountList, epTrDelay, mpa.myLambda, pg_loss_grad_norm)

    print('Indv Computation {}, {}'.format(rollTime, rollTime/(rollTime+sampling+actionTime+trainTime)))
    print('total sampling {}, {}'.format(sampling, sampling/(rollTime+sampling+actionTime+trainTime)))
    print('action time {}, {}'.format(actionTime, actionTime/(rollTime+sampling+actionTime+trainTime)))
    print('training time {}, {}'.format(trainTime, trainTime/(rollTime+sampling+actionTime+trainTime)))
