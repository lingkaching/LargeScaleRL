...
def episodeLoop(env, options, all_static_info, sess):
	...
	for steps in range(0, T):   
		...
		for nn in range(0, num_of_agents): 
			...
			if flags[nn] == False:	   
				...
				shaping = 0                       			
			rewards[nn].append(stepReward + shaping + penaltyAgents[nn])
	...
	resultSavePath = './vpg_log/results/'+str(options.mapSize)+'/agents'+str(options.numAgents)+'/'+options.experimentname
	with open(resultSavePath, 'a+') as f:
		f.write(json.dumps(states))
		f.write('\n')

	#Calculate empirical return, no credit assignment
	global_rewards = np.asarray(rewards).reshape((num_of_agents,-1))
	global_rewards = np.sum(global_rewards,axis=0)
	
	rets = []
	return_so_far = 0
	for t in range(len(global_rewards) - 1, -1, -1):
		return_so_far = global_rewards[t] + discount * return_so_far
		...
		for ss in range(0, len(oneAgent_states)-1):
			...
			if act != -1:                           
				if state in obsdict:
					...
					retdict[state].append(rets[ss]+entropys[nn][ss])                             
					nextretdict[state].append(rets[ss+1])					  
				else:
					obsdict[state] = []
					actdict[state] = []
					retdict[state] = []
...
def trainModel(options, cap, edges, g_1, polNNs, length, listofenvironments, T_min, T_max, num_of_agents, num_of_zones, GOALS, landmarks): 
	...
	with tf.Session(graph=g_1, config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
		...
		writer = tf.summary.FileWriter('./vpg_log/tflogs/'+str(options.mapSize)+'/agents'+str(options.numAgents)+'/'+options.experimentname, sess.graph)
		N = options.N
		T = options.T
		n_itr = options.n_itr
		discount = options.discount
		...
		for i in range(0,n_itr): 
			#new iteration
			print("Iteration->",i)
			resultSavePath = './vpg_log/results/'+str(options.mapSize)+'/agents'+str(options.numAgents)
			...
			result = pool.amap(episodeLoop, listofenvironments, [options]*len(listofenvironments), [all_static_info]*len(listofenvironments), [sess]*len(listofenvironments))
		...
		save_path = './vpg_log/trainedModel/'+str(options.mapSize)+'/agents'+str(options.numAgents)+'/'+options.experimentname
...
def main():
	...
	trainModel(options, cap, edges, g_1, polNNs, length, listofenvironments, T_min, T_max, num_of_agents, num_of_zones, GOALS, landmarks)
